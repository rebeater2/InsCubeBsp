/**
* @file GnssParseRaw.c in GnssParser
* @author rebeater
* @comment UB482 board Driver, decode binary.
...
uint32_t CalculateCRC32(const uint8_t *szBuf, int iSize) {
  int iIndex;
  uint32_t ulCRC = 0;
  for (iIndex = 0; iIndex < iSize; iIndex++) {
	ulCRC = aulCrcTable[(ulCRC ^ szBuf[iIndex]) & 0xff] ^ (ulCRC >> 8U);
  }
  return ulCRC;
}
#if 0
GnssMode ToNmeaMode(GnssType type) {
  GnssMode res = INVALID;
  switch (type) {
	case GNSS_TYPE_SINGLE: res = SPP;
	  break;
	case GNSS_TYPE_PSRDIFF: res = RTK_DGPS;
	  break;
	case GNSS_TYPE_L1_FLOAT:
	case GNSS_TYPE_IONOFREE_FLOAT:
	case GNSS_TYPE_NARROW_FLOAT: res = RTK_FLOAT;
	  break;
	case GNSS_TYPE_WIDE_INT:
	case GNSS_TYPE_NARROW_INT: res = RTK_FIX;
	  break;
	default:break;
  }
  return res;
 ...
 * @param data
 * @param gnss
 * @return 0:OK -1: CRC check failed
 */
int ParseRawBin(const uint8_t *data, GnssDataRaw *gnss) {
  ...
  BestPos *pos;
  int offset = 0;
  while (offset < MSG_MAX_LENGTH) {
	/*looking for head chars, 0xAA 0x44 0x12*/
	if (*(uint16_t *)&data[offset] != 0x44AA) { break; }//} && offset < MSG_MAX_LENGTH) { offset++; };
	header = (RawHeader *)(data + offset);
	if (header->sync[2] == 0x12) {
	  if (header->MessageID == MSG_Heading) {
		uint32_t crc = CalculateCRC32(data + offset, header->HeaderLength + header->MessageLength + 4);
		if (crc != 0) {
		  return -1;
		};
		heading = (GnssHeading *)(data + header->HeaderLength + offset);
		if (heading->SolStatus == SOL_COMPUTED) {
		  gnss->yaw = heading->Heading;
		  gnss->yaw_std_100	 = (short ) (100* heading->Hdgstddev);
		  gnss->pitch = heading->Pitch;
		  gnss->pitch_std_100 = (short)(100 * heading->PtchStdDev);
		} else {
#if DEBUG_INDOOR == 1
		  gnss->yaw = 6;
		  gnss->yaw_std_100 = 0;
...
#endif
		  gnss->pitch = -1.f;
		  gnss->pitch_std_100 = 0;
		}
	  }
	  if (header->MessageID == MSG_BEST_POS) {
		uint32_t crc = CalculateCRC32(data + offset, header->MessageLength + header->HeaderLength + 4);
		if (crc != 0) {
		  return -1;
		};
		pos = (BestPos *)(data + header->HeaderLength + offset);
		gnss->lat = pos->Lat;
		gnss->lon = pos->Lon;
		gnss->height = (float)pos->Hgt + pos->Undulation;
		gnss->pos_std[0] = (float)pos->LatStd;
		gnss->pos_std[1] = (float)pos->LonStd;
		gnss->pos_std[2] = (float)pos->HgtStd;
		gnss->ns = pos->SolnSvs;
		gnss->mode = ToNmeaMode(pos->PosType);
		gnss->week = header->Week;
		gnss->gpst = header->Second / 1000.0;
	  }
	  if (header->MessageID == MSG_RANGEA) {
	  }
	  offset += (header->MessageLength + header->HeaderLength + 4);
	} else {
	  break;
	}
  }
