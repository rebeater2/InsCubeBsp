...
static void fusion_push_out(system_time_t *tm, imu_msg_t *imu) {
    output_data_t out;
    out.header = 0xAA55AA55L;
    out.length = sizeof(out);
    out.tm = *tm;
    for (int i = 0; i < 3; i++) {
        out.acce[i] = imu->raw.acce[i];
        out.gyro[i] = imu->raw.gyro[i];
    }
    out.lat = 10.00001;
    out.lon = 1.3423234;
    out.height = 1.120f;
...
inline void msg_to_imu(imu_msg_t *msg, ImuData_t *imu_data, fusion_option_t *opt) {
    for (int i = 0; i < 3; ++i) {
...
_Noreturn void fusion_task(void *para) {
    ...
    while (1) {
        if (bsp_msg_get(task->imu_msg_que, &imu_msg, FUSION_TASK_MSG_DELAY) == bsp_os_ok) {
            ...
            fusion_update(TM_TO_DOUBLE(imu_msg.tm), &imu_data, SensorIMU);
        }

        /*push to output device*/
        if (imu_msg.tm.millisecond > tm_push_result.millisecond) {
            fusion_push_out(&imu_msg.tm, &imu_msg);
